...
int get_string(char* digits, int n);	//in read_input
/* REQUIRES
 *   n > 0.
...
int read_integer(char* digits, int n, int* num);	//in read_int
/* REQUIRES
...
int is_valid_integer(const char* digits);	//in read_int
...
int convert_to_integer(const char* digits);	//in read_int
...
int read_real(char* digits, int n, double * num);	//in read_double
/* REQUIRES
 *   n > 0, n is large enough to handle longest expected line of input. 
 *   Array elements digits[0], ...,  digits[n-1] exist.
 *   num points to a variable.
...
int is_valid_double(const char* digits);	//in read_double
/* REQUIRES: digits points to the beginning of a valid C string.
 *
 * PROMISES: Returns 1 if all characters in digits are 
...
double convert_to_double(const char *digits);	//in read_double
/* REQUIRES: digits points to a valid C string
